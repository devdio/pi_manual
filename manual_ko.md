...
-  방향과 속도를 동시 제어
로봇의 왼쪽 모터와 오른쪽 모터를 자유롭게 제어할 수 있는 함수입니다.
앞으로 회전은 'f', 뒤로 회전은 'b'로 설정하고, 속도는 0~255사이의 값을 지정할 수 있습니다.
...
#### 라인 센서
- 검은색은 1, 그 이외의 색은 0으로 판단해서 리턴합니다.
...
- 2개의 색상이 얼마나 비슷한지를 계산하는 함수입니다. 0에 가까울수록 두 색상이 가깝다는 의미입니다.
컬러센서가 인식한 색상이 무슨 색상인지 판단하기 위해서는 기준이 되는 색상과 컬러센서가 측정한 색상과의 거리를 측정해서 일정한 값보다 크지 않으면 같은 색상이라고 판단할 수 있습니다. 
```
white = (255, 255, 255)
black = (0, 0, 0)
ret = kb.diff_color(white, black)
print("색상 차이 : ", ret)
